...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        boton4.setOnClickListener {
            val admin = AdminSQliteOpenHelper(this, "administracion", null, 1)
            val bd = admin.writableDatabase
            val cant = bd.delete("articulo", "codigo=${et1.text.toString()}", null)
            bd.close()
            et1.setText("")
            et2.setText("")
            et3.setText("")
            if (cant == 1){
                Toast.makeText(this, "Se borro el articulo con dicho  codigo", Toast.LENGTH_SHORT)
                    .show()
            }else{
                Toast.makeText(this,"No exite este un articulo con dicho codigo", Toast.LENGTH_SHORT)
            }
        ...
        boton5.setOnClickListener {
            ...
            registro.put("precio",et3.toString())
            val cant = db.update("articulos", registro," codigo=${et1.text.toString()}",null)
            db.close()
            if (cant==1){
                Toast.makeText(this,"Se modificaron los datos",Toast.LENGTH_SHORT).show()
            }else{
                Toast.makeText(this,"no existe un articulo con el codigo  ingreso",Toast.LENGTH_SHORT).show()
            }
        }
    }
